...
object TransactionSpec extends DefaultRunnableSpec {
  def spec = suite("TransactionSpec")(
    ...
    test("SELL Transaction instantiation from json") {
      ...
      assert(transaction.fee)(equalTo(FungibleData(BigDecimal(0.00103099), "WBNB"))) &&
      assert(transaction.value)(equalTo(Right(FungibleData(BigDecimal("0.176295631347800986"), "WBNB"))))
    }
  )
}
