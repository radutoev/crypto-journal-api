    ...
    def query(threshold: Instant) =
      Query
        ...
        .setFilter(
          PropertyFilter.le(
            "timestamp",
            TimestampValue.of(Timestamp.ofTimeSecondsAndNanos(threshold.getEpochSecond, threshold.getNano))
          )
        )
        .build()
...
object DatastorePaginationRepo {
  lazy val layer
    : URLayer[Has[Datastore] with Has[DatastoreConfig] with Clock with Logging, Has[DatastorePaginationRepo]] =
